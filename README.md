...
Right now that means a multi-vendor commerce platform at **TechPixe India** — React on both sides of the fence, real vendors, real bugs that only show up after the third deploy. Before that, Astro Ratan: a chatbot that had to actually remember the last thing you said, which turned out to be a Redis problem dressed up as an AI problem. Before that, a booking system where the whole job was making sure two people never grabbed the same slot at the same second.
